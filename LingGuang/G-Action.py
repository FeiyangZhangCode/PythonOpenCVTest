...
imu_baud = 9600
imu_timeout = 0.05
laser_com = '/dev/ttyUSB0'
laser_baud = 115200
laser_timeout = 0.005
se = serial.Serial('/dev/ttyUSB1', 115200, timeout=0.1)
...
# 调用相机获取图片进行测距
def distance_get(q_cap, q_yi, q_img, q_temp, q_v2a, q_v2s, cap_id, file_address):
    # 根据相机编号分配模型参数
    global para_lines
...
# 读取IMU数据
def imu_get(q_ia, q_im, q_y, q_c, lock_ser, file_address):
    global glo_is_init
...
def multi_calc(q_img, q_temp, q_v2s, q_i, q_las, q_c, file_address):
    cv2.waitKey(1000)
    print('calc start')
    # 根据相机编号分配模型参数
    ...
    imu_roll = 0.0
    imu_pitch = 0.0
    cam_yaw = side_left = side_right = -999.9
    ac_ctl = [0]
    dis_laser = 999.9
...
# 校正偏航角
def correct_yaw(now_yaw, target_yaw, cmd_56, q_i, q_v2a, q_c, q_ci, file_address):
    ...
    for i in range(0, 3, 1):
        ...
        # 获取视觉数据
        if not q_v2a.empty():
            cam_yaw, side_l, side_r = q_v2a.get()
            if side_l != -999.9:
...
# 校正偏移距离
def correct_deviation(dis_dev, target_l, target_r, cmd_56, q_i, q_v2a, q_c, q_ci, file_address):
    global rec_side_left, rec_side_right
    is_right = False
    imu_yaw = 0.0
    side_l = -999.9
    ...
    if move_dis < 0:
        ...
        is_right = False
        rot_angle = -30.0
    get_correct_err = correct_yaw(now_yaw, rot_angle, cmd_56, q_i, q_v2a, q_c, q_ci, file_address)
    if get_correct_err > 90:
        return get_correct_err
    ...
    if cam_yaw != -999.9 or 0.0:
        ...
        now_yaw = imu_yaw
    get_straight_err = correct_yaw(now_yaw, 0.0, cmd_56, q_i, q_v2a, q_c, q_ci, file_address)
...
# 前后清洗
def go_wash(wash_speed, cmd_56, q_i, q_c, q_ci, q_v2a, lock_ser, file_address, loop_times, wash_left, wash_right, q_l):
    global rec_side_left, rec_side_right
    ...
    while get_state == 0 and (loop_num <= loop_times or loop_times == -1):
        ...
        if keyboard.is_pressed('b'):    # 急停按钮，标识报错并跳出
            ...
            get_state = 96
            break
        elif keyboard.is_pressed('e') or dis_laser <= laser_threshold:      # 手动或测距到边
            get_error = single_action(hex_allStop, q_c, q_ci, file_address)
            get_state = 1
            break
        elif abs(now_yaw) >= correct_thre_yaw:      # 偏航角过大，直接偏航校正
            print('偏航' + str(now_yaw))
            get_state = correct_yaw(now_yaw, 0.0, cmd_56, q_i, q_v2a, q_c, q_ci, file_address)
            if get_state > 90:
                get_error = single_action(hex_allStop, q_c, q_ci, file_address)
                return get_state
            cam_yaw = 0.0
            ...
            get_state = correct_deviation(dis_deviation, wash_left, wash_right, cmd_56, q_i, q_v2a, q_c, q_ci,
                                          file_address)
            if get_state > 90:
                get_error = single_action(hex_allStop, q_c, q_ci, file_address)
                return get_state
...
# 到边调头
def turn_around(cmd_56, q_i, q_c, q_ci, q_v2a, lock_ser, file_address):
    get_state = 0
...
# 到边后“U”型转向
def u_turn(to_left, cmd_56, q_i, q_c, q_ci, q_v2a, q_l, lock_ser, file_address):
    get_state = 0
...
# 到达终点后返回
def end_back(cmd_56, q_i, q_c, q_ci, q_v2a, q_l, lock_ser, file_address):
    get_state = 0
    cmd_24_rotateSpeed = CVFunc.trans_speed('40')
    cmd_24_moveSpeed = CVFunc.trans_speed('40')
    hex_turnRight = CVFunc.set_order(cmd_0_head + cmd_13_front + cmd_24_rotateSpeed + cmd_13_back + cmd_24_rotateSpeed + cmd_56)
...
# 单次L型转向
def l_turn(to_left, cmd_56, q_i, q_c, q_ci, q_v2a, q_l, lock_ser, file_address):
    get_state = 0
    cmd_24_rotateSpeed = CVFunc.trans_speed(str(int_turnSpeed))
    hex_turnRight = CVFunc.set_order(cmd_0_head + cmd_13_front + cmd_24_rotateSpeed + cmd_13_back + cmd_24_rotateSpeed + cmd_56)
    hex_turnLeft = CVFunc.set_order(cmd_0_head + cmd_13_back + cmd_24_rotateSpeed + cmd_13_front + cmd_24_rotateSpeed + cmd_56)
    ...
    if cam_yaw != -999.9 or 0.0:
        ...
        if cam_yaw != -999.9 or 0.0:
            ...
            loop_times = 0
            cmd_24_correctSpeed = CVFunc.trans_speed('10')
            hex_correctLeft = CVFunc.set_order(
                cmd_0_head + cmd_13_back + cmd_24_correctSpeed + cmd_13_front + cmd_24_correctSpeed + cmd_56)
            hex_correctRight = CVFunc.set_order(
                cmd_0_head + cmd_13_front + cmd_24_correctSpeed + cmd_13_back + cmd_24_correctSpeed + cmd_56)

            if to_left and (-120 < imu_yaw < -60):
                while (-120 < imu_yaw < -100) or (-80 < imu_yaw < -60):
                    if keyboard.is_pressed('b'):  # 急停按钮，标识报错并跳出
                        get_error = single_action(hex_allStop, q_c, q_ci, file_address)
                        get_state = 96
                        return get_state
                    ...
                    if -120 < imu_yaw < -100:
                        get_state = single_action(hex_correctRight, q_c, q_ci, file_address)
                    elif -80 < imu_yaw < -60:
                        get_state = single_action(hex_correctLeft, q_c, q_ci, file_address)
                    elif -100 <= imu_yaw <= -80:
                        print('完成IMU校准')
            ...
            elif not to_left and 60 < imu_yaw < 120:
                while (60 < imu_yaw < 80) or (100 < imu_yaw < 120):
                    if keyboard.is_pressed('b'):  # 急停按钮，标识报错并跳出
                        get_error = single_action(hex_allStop, q_c, q_ci, file_address)
                        get_state = 96
                        return get_state
...
# 执行自动控制
def autocontrol_run(q_i, q_v2a, q_c, q_ci, q_l, lock_ser, file_address):
...
# 2.单个动作测试
        ...
        while is_oneAction:
            ...
            elif keyboard.is_pressed('l'):  # 测试L型转弯
                print('L型左转')
                get_one_err = l_turn(True, cmd_56, q_i, q_c, q_ci, q_v2a, q_l, lock_ser, file_address)
            elif keyboard.is_pressed(';'):  # 测试L型转弯
                print('L型右转')
                get_one_err = l_turn(False, cmd_56, q_i, q_c, q_ci, q_v2a, q_l, lock_ser, file_address)
        ...
        while (not need_back) and (loop_num < loop_time or loop_time == -1):
            ...
            if side_l != -999.9 or side_r != -999.9:
                ...
                if abs(dis_deviation) > 50.0:
                    ...
                    get_state = correct_deviation(dis_deviation, wash_left_dis, wash_right_dis, cmd_56, q_i, q_v2a,
            ...
            get_state = go_wash(int_washSpeed, cmd_56, q_i, q_c, q_ci, q_v2a, lock_ser, file_address, wash_loops, wash_left_dis, wash_right_dis, q_l)
            if get_state == 98:
                print('无反馈，结束运行')
                no_feedBack = True
                break
            ...
            # 3.4.到达位置，调头
            get_state = turn_around(cmd_56, q_i, q_c, q_ci, q_v2a, lock_ser, file_address)
            if get_state == 98:
                print('无反馈，结束运行')
                no_feedBack = True
                break
            ...
            # # 3.4 根据循环次数判断左移还是右移
            # if loop_num % 2 == 0:
            #     get_state = u_turn(True, cmd_56, q_i, q_c, q_ci, q_v2a, q_l, lock_ser, file_address)
            # else:
            #     get_state = u_turn(False, cmd_56, q_i, q_c, q_ci, q_v2a, q_l, lock_ser, file_address)
            # if get_state == 98:
            #     print('无反馈，结束运行')
            #     no_feedBack = True
            #     break
            ...
            # if need_back:
            #     print('完成清洗，返回')
            #     get_state = end_back(cmd_56, q_i, q_c, q_ci, q_v2a, q_l, lock_ser, file_address)
            #     if get_state == 98:
            #         print('无反馈，结束运行')
            #         no_feedBack = True
            #         break
...
if __name__ == '__main__':
    ...
    processes.append(
        mp.Process(target=distance_get, args=(
            queue_camera, queue_yaw_imu, queue_image, queue_temp, queue_vision2action, queue_vision2show, camera_id, str_fileAddress)))
    # IMU测姿态
    processes.append(
        mp.Process(target=imu_get, args=(
            queue_imu_auto, queue_imu_main, queue_yaw_imu, queue_control_imu, lock, str_fileAddress)))
    # 视觉感知融合
    processes.append(
        mp.Process(target=multi_calc, args=(
            queue_image, queue_temp, queue_vision2show, queue_imu_main, queue_laser_main, queue_control, str_fileAddress)))
    # 自动运行
    processes.append(
        mp.Process(target=autocontrol_run, args=(
            queue_imu_auto, queue_vision2action, queue_control, queue_control_imu, queue_laser_auto, lock, str_fileAddress)))
    # 激光测距
