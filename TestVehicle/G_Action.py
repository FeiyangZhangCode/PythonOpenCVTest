...
cmd_24_rotate = CVFunc.trans_speed('40')
cmd_5_stop = '00'
cmd_5_front = '01'
cmd_5_back = '02'
cmd_6_stop = '00'
...
# 右旋
hex_rotateRight = CVFunc.set_order(cmd_0_head + cmd_13_front + cmd_24_rotate + cmd_13_back + cmd_24_rotate + cmd_5_stop + cmd_6_stop)
# 左旋
hex_rotateLeft = CVFunc.set_order(cmd_0_head + cmd_13_back + cmd_24_rotate + cmd_13_front + cmd_24_rotate + cmd_5_stop + cmd_6_stop)
# 右前转
...
def control_communication(hex_order, q_v2a, q_m2a, q_a2m, q_c, q_ci):
    ...
    if sensor_list[0] < 90:
        if (sensor_list[0] == 1 or sensor_list[1] == 1) and (sensor_list[2] == 0 and sensor_list[3] == 0):
            get_state = 1
        elif (sensor_list[2] == 1 or sensor_list[3] == 1) and (sensor_list[0] == 0 and sensor_list[1] == 0):
            get_state = 2
        elif sensor_list[0] == sensor_list[1] == sensor_list[2] == sensor_list[3] == 0:
            get_state = 0
        else:
            get_state = 3
...
def go_wash(wash_speed, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, lock_ser, file_address, loop_times, wash_left, wash_right):
    ...
    while get_state == 0 and (loop_num <= loop_times or loop_times == -1):
        ...
        # 3.执行清洗移动
        get_state, cam_yaw, side_l, side_r, dis_laser, imu_roll, imu_pitch, imu_yaw = control_communication(
...
# 90度“L”型转向动作
def l_turn(to_left, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, lock_ser, file_address):
    get_state = 0
    moveSpeed_uTurn = 40
    cmd_24_rotateSpeed = CVFunc.trans_speed(str(int_turnSpeed))
    cmd_24_moveSpeed = CVFunc.trans_speed(str(moveSpeed_uTurn))
    ...
    if now_yaw != -999.9:
        ...
        if get_state > 0:
            get_error, cam_yaw, dis_left, dis_right, dis_laser, imu_roll, imu_pitch, imu_yaw = control_communication(
                hex_allStop, q_v2a, q_m2a, q_a2m, q_c, q_ci)
            return get_state
    # elif 80 < abs(imu_yaw) < 100:
    #     if to_left and imu_yaw < 0:
    #
    #     elif not to_left and imu_yaw > 0:
    #         pass
    #     else:
    #         print('IMU识别错误')
    # else:
...
# 直行指令
def go_straight(move_type, move_speed, move_times, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address):
    global rec_side_left, rec_side_right
    get_state = 0
    imu_yaw = 0.0
    imu_pitch = 0.0
    ...
    # 视觉偏航与IMU偏航相符标识位。如果相符，无视觉时靠IMU；如果不符，无视觉时置零。
    cy_equal_iy = False
    while get_state == 0 and loop_num <= loop_times:
        loop_num += 1
        # 键盘输入急停
        if keyboard.is_pressed('b'):
            get_state, cam_yaw, dis_left, dis_right, dis_laser, imu_roll, imu_pitch, imu_yaw = control_communication(
        ...
        get_state, cam_yaw, side_l, side_r, dis_laser, imu_roll, imu_pitch, imu_yaw = control_communication(
            hex_goStraight, q_v2a, q_m2a, q_a2m, q_c, q_ci)
        if get_state > 90:
            get_error, cam_yaw, dis_left, dis_right, dis_laser, imu_roll, imu_pitch, imu_yaw = control_communication(
                hex_allStop, q_v2a, q_m2a, q_a2m, q_c, q_ci)
            return get_state
        ...
        # 手动或测距到边，除去退后状态
        if (keyboard.is_pressed('r') or dis_laser <= laser_threshold) and move_type != 3:      # 手动或测距到边
            get_state, cam_yaw, dis_left, dis_right, dis_laser, imu_roll, imu_pitch, imu_yaw = control_communication(
                hex_allStop, q_v2a, q_m2a, q_a2m, q_c, q_ci)
            get_state = 1
            break
...
# 执行自动控制
def autocontrol_run(q_v2a, q_m2a, q_a2m, q_c, q_ci, lock_ser, file_address, q_k):
    global panel_width, rec_side_left, rec_side_right

    # 设置清洗参数
    loop_time = -1  # 单向算作1次
    wash_loops = -1  # 设置前后清洗的移动距离限定，-1代表无限定。每次200ms，5次相当于1秒
    wash_left_dis = 100.0  # 左侧边界距离
    wash_right_dis = panel_width - vehicle_width - wash_left_dis  # 左侧边界距离
    ...
    while True:
        ...
        while is_oneAction:
            # 发送数据
            k_input = cv2.waitKey() & 0xFF
            if k_input == ord('w'):  # 向前移动，刮板和水系统停止
                # print('前进')
                hex_order = hex_Front
            elif k_input == ord('s'):  # 向后移动，刮板和水系统停止
                # print('后退')
                hex_order = hex_Back
            elif k_input == ord('a'):  # 向左旋转，刮板和水系统停止
                # print('左旋')
                hex_order = hex_rotateLeft
            elif k_input == ord('d'):  # 向右旋转，刮板和水系统停止
                # print('右旋')
                hex_order = hex_rotateRight
            elif k_input == ord('q'):  # 前行并左转，刮板和水系统停止
                # print('左前转')
                hex_order = hex_FrontLeft
            elif k_input == ord('e'):  # 前行并左转，刮板和水系统停止
                # print('右前转')
                ...
                print('校正偏航')
                get_state = correct_yaw(cam_yaw, 0.0, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address)
                if get_state == 98:
                    print('无反馈，结束运行')
                    no_feedBack = True
                elif get_state == 96:
                ...
                print('右L')
                get_state = l_turn(False, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, lock_ser, file_address)
                if get_state == 98:
                    print('无反馈，结束运行')
                    no_feedBack = True
                elif get_state == 96:
                    ...
                    print('防跌落故障')
                hex_order = hex_allStop
            elif k_input == ord('u'):  # 动作测试:
                print('左L')
                get_state = l_turn(True, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, lock_ser, file_address)
                if get_state == 98:
                    print('无反馈，结束运行')
                    no_feedBack = True
                elif get_state == 96:
                ...
                print('加速测试')
                get_state = go_straight(1, 50, 10, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address)
                if get_state == 98:
                    print('无反馈，结束运行')
                    no_feedBack = True
                elif get_state == 96:
                ...
                print('减速测试')
                get_state = go_straight(2, 50, 10, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address)
                if get_state == 98:
                    print('无反馈，结束运行')
                    no_feedBack = True
                elif get_state == 96:
                    ...
                    print('防跌落故障')
                hex_order = hex_allStop
            elif k_input == ord('j'):  # 动作测试:
                print('直行次数测试')
                get_state = go_straight(0, 50, 5, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address)
                if get_state == 98:
                    print('无反馈，结束运行')
                    no_feedBack = True
                elif get_state == 96:
                ...
                get_state = go_straight(3, 10, 10, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address)
                if get_state == 98:
                    print('无反馈，结束运行')
                    no_feedBack = True
                elif get_state == 96:
            ...
            #     hex_order = hex_allStop
            #     is_oneAction = False
            # elif keyboard.is_pressed('t'):  # 动作测试:
            #     print('校正偏航')
            #     get_state = correct_yaw(cam_yaw, 0.0, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address)
            #     if get_state == 98:
            #         print('无反馈，结束运行')
            ...
            #         print('防跌落故障')
            #     hex_order = hex_allStop
            # elif keyboard.is_pressed('y'):  # 动作测试:
            #     print('右L')
            #     get_state = l_turn(False, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, lock_ser, file_address)
            #     if get_state == 98:
            #         print('无反馈，结束运行')
            ...
            #         print('防跌落故障')
            #     hex_order = hex_allStop
            # elif keyboard.is_pressed('u'):  # 动作测试:
            #     print('左L')
            #     get_state = l_turn(True, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, lock_ser, file_address)
            #     if get_state == 98:
            #         print('无反馈，结束运行')
            ...
            #         print('防跌落故障')
            #     hex_order = hex_allStop
            # elif keyboard.is_pressed('g'):  # 动作测试:
            #     print('加速测试')
            #     get_state = go_straight(1, 50, 10, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address)
            #     if get_state == 98:
            #         print('无反馈，结束运行')
            ...
            #         print('防跌落故障')
            #     hex_order = hex_allStop
            # elif keyboard.is_pressed('h'):  # 动作测试:
            #     print('减速测试')
            #     get_state = go_straight(2, 50, 10, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address)
            #     if get_state == 98:
            #         print('无反馈，结束运行')
            ...
            #         print('防跌落故障')
            #     hex_order = hex_allStop
            # elif keyboard.is_pressed('j'):  # 动作测试:
            #     print('直行次数测试')
            #     get_state = go_straight(0, 50, 5, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address)
            #     if get_state == 98:
            #         print('无反馈，结束运行')
            ...
            #         print('防跌落故障')
            #     hex_order = hex_allStop
            # elif keyboard.is_pressed('k'):  # 动作测试:
            #     get_state = go_straight(3, 10, 10, cmd_56, q_v2a, q_m2a, q_a2m, q_c, q_ci, file_address)
            #     if get_state == 98:
            #         print('无反馈，结束运行')
            #         no_feedBack = True
