...
def get_red(img_rgb):
    ...
    img_height = int(img_rgb.shape[0])
    img_width = int(img_rgb.shape[1])
    # img_new = np.zeros((img_height, img_width, 3), np.uint8)
    gra_new = np.zeros((img_height, img_width), np.uint8)
    # 提取红色部分
    b_threshold = 155
    g_threshold = 155
    r_threshold = 195
    ...
    for j_ut in range(0, img_width, 1):
        for i_ut in range(0, img_height, 1):
            ...
            if b_value < b_threshold and g_value < g_threshold and r_value > r_threshold:
                # img_new[i_ut, j_ut] = (255, 255, 255)
                gra_new[i_ut, j_ut] = 255
            else:
