    ...
    while True:
        ...
        if not q0.empty():
            ...
            for i in range(0, 3, 1):
                c0_list[i] = temp_list[i]

            se_send = 'C0F' + str(c0_list[0]) + 'L' + str(c0_list[1]) + 'R' + str(c0_list[2])
            lock_ser.acquire()
            se.write(se_send.encode())
            print(se_send)
            lock_ser.release()
        ...
        if not q1.empty():
            ...
            temp_list = q1.get()
            for i in range(0, 3, 1):
                c1_list[i] = temp_list[i]
            se_send = 'C1F' + str(c1_list[0]) + 'L' + str(c1_list[1]) + 'R' + str(c1_list[2])
            lock_ser.acquire()
            se.write(se_send.encode())
            print(se_send)
            lock_ser.release()
