...
while cap_num == 0:
    ...
    if cap_num == 1:
        # 第一个摄像头0
        cap0 = cv2.VideoCapture(cap_ID[0], cv2.CAP_DSHOW)
        ...
        print(get_camera_data(cap0, cap_ID[0]))
        # 第二个摄像头1
        cap1 = cv2.VideoCapture(cap_ID[1], cv2.CAP_DSHOW)
...
while True:
    ...
    if cap_num == 1:
        ...
        # 运行边界识别子程序
        frame0_distance = get_distance(frame0, 'C0:')
        # 存储边界图
        ...
        file_rec.write(str_Time + ',' + str(loop_num) + ',C0+C1' + '\n')
        # 运行边界识别子程序
        frame0_distance = get_distance(frame0, 'C0:')
        frame1_distance = get_distance(frame1, 'C1:')
        # 存储边界图
        # cv2.imwrite(str_fileAddress + str_Time + "-C0D" + '.jpg', frame0_distance)
        # cv2.imwrite(str_fileAddress + str_Time + "-C1D" + '.jpg', frame1_distance)

        cv2.namedWindow('Camera-0', cv2.WINDOW_NORMAL)
        cv2.imshow('Camera-0', frame0_distance)
        cv2.namedWindow('Camera-1', cv2.WINDOW_NORMAL)
        cv2.imshow('Camera-1', frame1_distance)
    elif cap_num == 0:
        print('未找到摄像头')
        break
    ...
    if loop_num % 5 == 0:
        ina_mess = INA219.get_ina219_data()
        file_rec.write(str_Time + ',' + ina_mess)
